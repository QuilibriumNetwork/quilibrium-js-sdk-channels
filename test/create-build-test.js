...
try {
  execSync('yarn build', { stdio: 'inherit', cwd: path.join(__dirname, '..') });
...
console.log('📝 Generating test-build.html...');
...
<body>
    ...
    <div class="build-info">
        <h4>Build Info</h4>
        <div>✅ Using built CSS from dist/index.css</div>
        <div>🎯 Testing for horizontal scroll fix</div>
    ...
    <script type="module">
        ...
        document.getElementById('root').innerHTML = \`
            <div style="text-align: center; color: white; margin-top: 100px; padding: 20px;">
                <h1 style="font-size: 2.5rem; margin-bottom: 20px;">🧪 Build Test</h1>
                ...
                <div style="background: rgba(255,255,255,0.1); padding: 20px; border-radius: 12px; max-width: 600px; margin: 0 auto;">
                    <p style="font-size: 1.1rem; margin-bottom: 15px;">
                        ✅ This test uses the actual built CSS from <code>dist/index.css</code>
                    </p>
                    <p style="font-size: 1.1rem; margin-bottom: 15px;">
                        🎯 Verify that no horizontal scrolling occurs in any modal state
                    </p>
        ...
        setTimeout(() => {
            console.log('🚀 Auto-testing import modal...');
...
const testFilePath = path.join(__dirname, 'test-build.html');
fs.writeFileSync(testFilePath, testHtml);

console.log('✅ test-build.html generated successfully');
console.log('🚀 Starting server on http://localhost:3001');

try {
  execSync('python3 -m http.server 3001', { stdio: 'inherit', cwd: __dirname });
} catch (error) {
  console.log('\n👋 Server stopped');
}